...
class Mysqldb:
	...
	def selectUncrawledLinkList(self, limit=100):
		selectSQL = "SELECT id, link_str, domain_id FROM link WHERE crawled = 0 LIMIT %s"
		...
		try:
			with self.connection.cursor() as cursor:
				...
				for row in cursor:
					links[row['id']] = [row['link_str'], row['domain_id']]

				cursor.close()
		except: 
			print("error selectUncrawledLinkList %s " % (cursor._last_executed))

		return links

	def selectDomainLinks(self, domainId, limit=1):
		selectSQL = "SELECT id, link_str FROM `link` WHERE `crawled` = 0 and domain_id = %s LIMIT %s"
		links = {}
		...
		try:
			...
			print("error selectDomainLinks: %s " % (cursor._last_executed))

		return links

	def selectDomainLinksFromIds(self, domainIds):
		selectSQL = "SELECT id, link_str FROM `link` WHERE `crawled` = 0 and domain_id in (%s)"
		in_p=', '.join(list(map(lambda x: '%s', domainIds)))
		selectSQL = selectSQL % in_p
	...
	def insertArticleTFIDF(self, wordId, articleId, tfidf):
		insertSQL = "INSERT INTO article_tfidf (word_id, tfidf, article_id) VALUES (%s, %s, %s)"
		article_tfidf_id = None

		try:
			with self.connection.cursor() as cursor:
				cursor.execute(insertSQL, (wordId, tfidf, articleId))
				article_tfidf_id = cursor.lastrowid

				cursor.close()
		except:
			print("error insertArticleTFIDF %s" % (cursor._last_executed))

		self.connection.commit()

		return article_tfidf_id
	...
	def selectDomainFromHighestRelatedTFIDF(self):
		selectSQL = "SELECT related_domain_id FROM link join article on article.link_id = link.id join domain_relation on link.domain_id = domain_relation.base_domain_id order by article.tfidf limit 1"
		...
		try:
			with self.connection.cursor() as cursor:
				...
				for row in cursor:
					domainId = row['related_domain_id']
		except:
			print("error selectDomainFromHighestRelatedTFIDF")
		return domainId

	def selectDomainFromId(self, linkId):
		selectSQL = "SELECT link.domain_id, domain_extension.extension FROM link INNER JOIN domain_extension ON link.domain_extension_id=domain_extension.id WHERE link.id = %s"
		domain = None
		extension = None
		...
		try:
			...
			print("error selecting domain from id: %s " % (cursor._last_executed))

		return domain, extension

	def selectOnlyDomainFromId(self, linkId):
		selectSQL = "SELECT link.domain_id FROM link WHERE link.id = %s"
		domain = None
	...
	def selectDomainFromArticle(self, articleId):
		selectSQL = "SELECT domain_id FROM link join article on link.id = article.link_id WHERE article.id = %s"
		domainId = None
		...
		try:
			...
			print("error selectDomainFromArticle: %s" % (e))
	...
	def selectRelatedFromBase(self, baseDomainId):
		...
		try:
			with self.connection.cursor() as cursor:
				...
				for row in cursor:
					domainRelationIds.append(row['related_domain_id'])
				cursor.close()
		except: 
			print("error selecting domain relation id")

		return domainRelationIds
	...
	def selectWordCount(self):
		selectSQL = "SELECT count(*) as count FROM word"
		...
		try:
			...
			print("error sselectWordCount: %s " % (cursor._last_executed))
	...
	def selectArticleWithNoTFIDF(self, limit=100):
		...
		try:
			...
			print("error selectArticleWithNoTFIDF: %s " % (cursor._last_executed))
	...
	def selectBaseArticlesFromWords(self, wordIds):
		selectSQL = "SELECT article.id FROM article_word join article on article_word.article_id = article.id join link on article.link_id = link.id WHERE article_word.word_id in (%s) and link.baselink = 1"
		in_p=', '.join(list(map(lambda x: '%s', wordIds)))
		selectSQL = selectSQL % (in_p)
		...
		try:
			with self.connection.cursor() as cursor:
				cursor.execute(selectSQL, wordIds)
				for row in cursor:
					articlesIds.append(row['id'])
				cursor.close()
		except: 
			print("error selectBaseArticlesFromWords: %s " % (cursor._last_executed))
	...
	def selectArticleFromWords(self, wordIds):
		...
		try:
			...
			print("error selectArticleFromWords: %s " % (cursor._last_executed))
	...
	def selectArticles(self, limitLow=0, batchSize=18446744073709551615):
		selectSQL = "SELECT id FROM article LIMIT %s,%s"

		articleIds = []

		try:
			with self.connection.cursor() as cursor:
				cursor.execute(selectSQL, (limitLow, batchSize))
				for row in cursor:
					articleIds.append(row['id'])
				cursor.close()
		except: 
			print("error selectArticles: %s " % (cursor._last_executed))
	...
	def selectArticlesByTime(self):
		...
		try:
			...
			print("error selectArticlesAndTime: %s " % (cursor._last_executed))
